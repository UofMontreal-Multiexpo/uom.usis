...
# Loading data
field_description = read.csv("./USIS_database/field_description.csv",
                             encoding = "UTF-8")
```
...
\blandscape
```{r schema, fig.cap="USIS database relational schema.", out.width="100%"}
...
text = substr_using_regex(field_description$Comments, regex_text)
text = substr(text, 2, nchar(text) - 1)
links = substr_using_regex(field_description$Comments, regex_link)
links = substr(links, 2, nchar(links) - 1)

# Create and display the table
flextable(field_description) %>%
  set_caption(caption = "Description of USIS main fields.") %>%
...
<!-- Inside columns, long table captions overflow the margins and off the page.
...
## Appendix D: U.S. territorial division reference table {#appendix-d}

```{r table-territories}
flextable(data.frame(ID   = rownames(US_territories),
                     Name = US_territories[, "name"],
                     Type = US_territories[, "type"])) %>%
  set_caption(caption = "Identifiers, names and types of the U.S. territorial divisions.") %>%
  set_table_properties(layout = "autofit")
```


<!-- Page break to prevent the link to appendix E from redirecting to the bottom
     of the previous page whereas the title is on the next one -->
\newpage
